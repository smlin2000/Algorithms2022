...
std::vector<int> searchShortestPath(Graph &G, int start, int destination) {
    ...
    distance[start] = 0; // Inititlize distance of start.
    parent[start] = -1; // Initialize parent of start.
    ...
    for (int i = 1; i <= G.n - 1; ++i) { // We loop over n-1 times as there will be not path has the number of edges > n-1
        ...
        visited[u] = true; // Set u as a visited vertex.
    ...
    std::vector<int> path;
    int u = destination; // We trace back from the destination.
    while (u != -1) {
        path.push_back(u);
        u = parent[u]; // Based on parent[u], we move back one step.
    }
